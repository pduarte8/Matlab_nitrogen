...
betaQ1     = [Results.quad_linear];
betaQ2     = [Results.quad_curvature];
...
for i = 1:nsec
    ...
    betaQ0 = R0.quad_intercept;
    betaQ1 = R0.quad_linear;
    betaQ2 = R0.quad_curvature;

    if isfinite(betaQ0) && isfinite(betaQ1) && isfinite(betaQ2) && nnz(isAW) >= 2
        xs = linspace(min(sal(isAW)), max(sal(isAW)), 50);
        xc = xs - mean(xs);
        ys = betaQ2*xc.^2+betaQ1*xc + betaQ0;
